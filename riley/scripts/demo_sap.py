...
import riley.protomodules.embeddings as em
from riley.protomodules.blendenpik import blendenpik_srct
import numpy as np
import math
import random
...
def demo_1():
    """Compare naive initialization to smart initialization

    For the high coherence matrices considered here (low not tested), the
    smart initialization does much better than the naive initialization
    (savings of 25% when theta=0.001, solve in a single iteration when theta=0,
    compared to 40+ iterations when theta=0 for naive initialization).

    NOTE: when the system is almost consistent, the normal equation error with
    smart init tends to be MUCH smaller than with naive init. I would expect
    the iteration count to be lower. But I suppose LSQR's termination criteria
    is doing more than checking relative normal equation error.
    """
    n_rows = 10000
    n_cols = 100
    maxit = n_cols
    arc = 50
    theta = 1 - 0.999
    A, b = overdetermined_ls_test_matrix_generator(n_rows, n_cols, theta,
    ...
    for seed in range(10):
        np.random.seed(seed)
        res0 = blendenpik_srct(A, b, d, tol, maxit)
        np.random.seed(seed)
        S = em.srct_embedding(d, A.shape[0])
        res1 = sap.sketch_and_precond(A, b, S, tol/10, maxit)

        err0 = np.linalg.norm(A.T @ (A @ res0[0]) - A.T @ b)
        err1 = np.linalg.norm(A.T @ (A @ res1[0]) - A.T @ b)
        print('\nNormal equation errors: \n\t Zero init:   %s \n\t Sketch init: %s\n' % (str(err0), str(err1)))
        print('\nIteration counts: \n\t Zero init:   %s \n\t Sketch init: %s\n' % (str(res0[2]), str(res1[2])))
...
if __name__ == '__main__':
    ...
    theta = 1 - 0.99999999
    A, b = overdetermined_ls_test_matrix_generator(n_rows, n_cols, theta,
                                                   coherence_type='low', added_row_count=arc,
                                                   fill_diagonal_method='arithmetic', a_cond=np.log2(1e4))
    tol = 1e-8
    d = int(1.1*n_cols)
    seed = 0
    for seed in range(10):
        ...
        np.random.seed(seed)
        S = em.srct_embedding(d, A.shape[0])
        res1 = sap.sketch_and_precond(A, b, S, tol, maxit)
