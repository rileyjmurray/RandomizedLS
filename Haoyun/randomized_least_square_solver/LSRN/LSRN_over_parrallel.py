...
def LSRN_over_parallel(A, b, tol=1e-8, gamma=2, iter_lim=1000, comm=MPI.COMM_WORLD):
    ...
    timing = {'randn_and_mult': 0.0, 'svd': 0.0, 'iter': 0.0, 'comm': 0.0, 'all': 0.0}
    ...
    if m > n:  # over-determined
        ...
        blk_sz = num_cores
        inputs = range(int(ceil(1.0 * s / blk_sz)))

        def parallel_guassian(i):
            blk_begin = i * blk_sz
        ...
        tic_randn_and_mult = perf_counter()

        pool = multiprocessing.Pool(num_cores)
        A_tilde = pool.map(parallel_guassian, [i for i in inputs])
        # A_tilde = np.array(Parallel(n_jobs=num_cores)(delayed(parallel_guassian)(i) for i in inputs)).reshape(s, n)
        timing['randn_and_mult'] += perf_counter() - tic_randn_and_mult
